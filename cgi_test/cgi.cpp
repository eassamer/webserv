...
/*   Updated: 2022/11/29 14:20:56 by bboulhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Cgi::Cgi(const server &serv, const location &loc){
	this->serv = serv;
	this->loc = loc;
}
...
void Cgi::env_init()
{
	...
	this->env["SERVER_SOFTWARE"] = "name of our server";
...
void Cgi::execute_cgi()
{
	...
	else if (pid == 0)
	{
		...
		execve(this->loc.get_cgi_path().c_str(), NULL, this->c_env);
		std::cout << "Error\n";
